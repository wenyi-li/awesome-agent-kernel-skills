...
def gemv_alloc_reducer(
    a, x, block_M=128, block_N=128, num_stages=2, threads=256, dtype: T.dtype = T.float16, accum_dtype: T.dtype = T.float
):
    M, N = T.const("M, N")
...
def main(do_bench: bool = True):
    ...
    N, K = args.n, args.k
    check_correctness_and_bench(naive_gemv.compile(N=N, K=K, BLOCK_N=128, BLOCK_K=128), N, K, do_bench=do_bench)
    check_correctness_and_bench(naive_splitk_gemv.compile(N=N, K=K, BLOCK_N=32, BLOCK_K=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv.compile(N=N, K=K, BLOCK_N=32, BLOCK_K=32, reduce_threads=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv_vectorized.compile(N=N, K=K, BLOCK_N=2, reduce_threads=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(splitk_gemv_vectorized_tvm.compile(N=N, K=K, BLOCK_N=2, reduce_threads=32), N, K, do_bench=do_bench)
    check_correctness_and_bench(gemv_alloc_reducer.compile(M=N, N=K, block_M=128, block_N=128), N, K, do_bench=do_bench)

    print("Test passed!")

    if do_bench:
        best_result = get_autotuned_kernel.compile(N=N, K=K)
        best_config = best_result.config
        kernel = splitk_gemv_vectorized_tvm.compile(N=N, K=K, **best_config)
        profiler = kernel.get_profiler()
        ...
        kernel = gemv_alloc_reducer.compile(M=N, N=K)
        profiler = kernel.get_profiler()
...
def run_regression_perf():
    ...
    latency = 0.0
    kernel_list = [
        naive_gemv.compile(N=N, K=K, BLOCK_N=128, BLOCK_K=128),
        naive_splitk_gemv.compile(N=N, K=K, BLOCK_N=32, BLOCK_K=32),
        splitk_gemv.compile(N=N, K=K, BLOCK_N=32, BLOCK_K=32, reduce_threads=32),
        splitk_gemv_vectorized.compile(N=N, K=K, BLOCK_N=2, reduce_threads=32),
        splitk_gemv_vectorized_tvm.compile(N=N, K=K, BLOCK_N=2, reduce_threads=32),
        gemv_alloc_reducer.compile(M=N, N=K, block_M=128, block_N=128),
    ]
