...
@tilelang.jit
def matmul_warp_specialize_copy_1_gemm_0(A, B, block_M, block_N, block_K, dtype=T.float16, accum_dtype=T.float32):
    M, N, K = T.const("M, N, K")
...
def main(M=16384, N=16384, K=16384):
    ...
    # cuda_source = matmul_warp_specialize_copy_1_gemm_0.get_kernel_source(a, b, block_M, block_N, block_K)
    # print("Generated CUDA kernel:\n", cuda_source)

    # Profile latency with kernel
    from tilelang.profiler import do_bench

    latency = do_bench(lambda: matmul_warp_specialize_copy_1_gemm_0(a, b, block_M, block_N, block_K))
...
def run_regression_perf(M=16384, N=16384, K=16384):
    ...
    return do_bench(lambda: matmul_warp_specialize_copy_1_gemm_0(a, b, block_M, block_N, block_K), backend="cupti")
