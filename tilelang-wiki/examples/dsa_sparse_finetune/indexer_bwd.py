...
def tl_indexer_bwd_impl(
    ...
    seq_len, batch_plus_one = T.dynamic("seq_len, batch_plus_one")
    dtype: str = BF16
    ...
    dIndexQ = T.empty(index_q_shape, dtype)
    dWeights = T.empty(weights_shape, dtype)
    dIndexK = T.empty(index_k_shape, dtype)
...
def indexer_bwd_interface(
    ...
    dq, dweights, dk = tl_indexer_bwd_impl(q, weights, k, attn_score, index_score, topk_indices, offsets, token_indices, heads, dim, topk)
