...
Environment variables can also be set statically in /etc/nccl.conf (for an administrator to set system-wide values) or in ${NCCL_CONF_FILE} (since 2.23; see below). For example, those files could contain:
...
## System configuration[](#system-configuration "Permalink to this heading")

### NCCL_SOCKET_IFNAME[](#nccl-socket-ifname "Permalink to this heading")

The `NCCL_SOCKET_IFNAME` variable specifies which IP interfaces to use for communication.

#### Values accepted[](#values-accepted "Permalink to this heading")
...
### NCCL_SOCKET_FAMILY[](#nccl-socket-family "Permalink to this heading")

The `NCCL_SOCKET_FAMILY` variable allows users to force NCCL to use only IPv4 or IPv6 interface.

#### Values accepted[](#id2 "Permalink to this heading")
...
### NCCL_SOCKET_RETRY_SLEEP_MSEC[](#nccl-socket-retry-sleep-msec "Permalink to this heading")
...
### NCCL_SOCKET_POLL_TIMEOUT_MSEC[](#nccl-socket-poll-timeout-msec "Permalink to this heading")
...
### NCCL_SOCKET_NTHREADS[](#nccl-socket-nthreads "Permalink to this heading")
...
### NCCL_NSOCKS_PERTHREAD[](#nccl-nsocks-perthread "Permalink to this heading")
...
### NCCL_CROSS_NIC[](#nccl-cross-nic "Permalink to this heading")
...
### NCCL_IB_HCA[](#nccl-ib-hca "Permalink to this heading")
...
### NCCL_IB_TIMEOUT[](#nccl-ib-timeout "Permalink to this heading")

The `NCCL_IB_TIMEOUT` variable controls the InfiniBand Verbs Timeout.

The timeout is computed as 4.096 µs * 2 ^ _timeout_ , and the best value is dependent on the size of the network. Increasing that value can help on very large networks, for example, if NCCL is failing on a call to _ibv_poll_cq_ with error 12.

For convenience and clarity, the following table shows some of the commonly used values and the resulting timeout. For more information, see section 12.7.34 of the InfiniBand specification Volume 1 (Local Ack Timeout).

Note that the total time spent waiting for a response is the product of `NCCL_IB_TIMEOUT` and `NCCL_IB_RETRY_CNT`.
...
### NCCL_IB_RETRY_CNT[](#nccl-ib-retry-cnt "Permalink to this heading")

(since 2.1.15)

The `NCCL_IB_RETRY_CNT` variable controls the InfiniBand retry count. Total time spent retrying is determined by the product of the retry count and the timeout. For example, the default values of `NCCL_IB_TIMEOUT=20` and `NCCL_IB_RETRY_CNT=7` result in approximately 30 seconds of waiting (7 retries, each lasting 4.3 seconds) before a network error is raised by NCCL.
...
### NCCL_IB_GID_INDEX[](#nccl-ib-gid-index "Permalink to this heading")

(since 2.1.4)

The `NCCL_IB_GID_INDEX` variable defines the Global ID index used in RoCE mode. See the InfiniBand _show_gids_ command in order to set this value.
...
### NCCL_IB_ADDR_FAMILY[](#nccl-ib-addr-family "Permalink to this heading")
...
### NCCL_IB_ADDR_RANGE[](#nccl-ib-addr-range "Permalink to this heading")
...
### NCCL_IB_ROCE_VERSION_NUM[](#nccl-ib-roce-version-num "Permalink to this heading")
...
### NCCL_IB_SL[](#nccl-ib-sl "Permalink to this heading")
...
### NCCL_IB_TC[](#nccl-ib-tc "Permalink to this heading")
...
### NCCL_IB_FIFO_TC[](#nccl-ib-fifo-tc "Permalink to this heading")
...
### NCCL_IB_RETURN_ASYNC_EVENTS[](#nccl-ib-return-async-events "Permalink to this heading")
...
### NCCL_OOB_NET_ENABLE[](#nccl-oob-net-enable "Permalink to this heading")
...
### NCCL_OOB_NET_IFNAME[](#nccl-oob-net-ifname "Permalink to this heading")
...
### NCCL_UID_STAGGER_THRESHOLD[](#nccl-uid-stagger-threshold "Permalink to this heading")

(since 2.23) The `NCCL_UID_STAGGER_THRESHOLD` variable is used to trigger staggering of communications between NCCL ranks and the ncclUniqueId in order to avoid overflowing the ncclUniqueId. If the number of NCCL ranks communicating exceeds the specified threshold, the communications are staggered using the rank value (see NCCL_UID_STAGGER_RATE below). If the number of NCCL ranks per ncclUniqueId is smaller or equal to the threshold, no staggering is performed.
...
### NCCL_UID_STAGGER_RATE[](#nccl-uid-stagger-rate "Permalink to this heading")
...
### NCCL_NET[](#nccl-net "Permalink to this heading")
...
### NCCL_NET_PLUGIN[](#nccl-net-plugin "Permalink to this heading")
...
### NCCL_TUNER_PLUGIN[](#nccl-tuner-plugin "Permalink to this heading")
...
### NCCL_PROFILER_PLUGIN[](#nccl-profiler-plugin "Permalink to this heading")
...
### NCCL_ENV_PLUGIN[](#nccl-env-plugin "Permalink to this heading")
...
The `NCCL_ENV_PLUGIN` variable can be used to let NCCL load an external environment plugin. Set it to either a library name or a suffix string to choose among multiple NCCL environment plugins. This setting will cause NCCL to look for the environment plugin library using the following strategy:
  ...
  * If `NCCL_ENV_PLUGIN` is set to a library name, attempt loading that library (e.g. `NCCL_ENV_PLUGIN=/path/to/library/libfoo.so` will cause NCCL to try to load `/path/to/library/libfoo.so`);

  * If `NCCL_ENV_PLUGIN` is set to a suffix string, attempt loading `libnccl-env-<NCCL_ENV_PLUGIN>.so` (e.g. `NCCL_ENV_PLUGIN=foo` will cause NCCL to try to load `libnccl-env-foo.so` from the system library path);
...
### NCCL_IGNORE_CPU_AFFINITY[](#nccl-ignore-cpu-affinity "Permalink to this heading")
...
### NCCL_CONF_FILE[](#nccl-conf-file "Permalink to this heading")
...
### NCCL_DEBUG[](#nccl-debug "Permalink to this heading")
...
### NCCL_DEBUG_FILE[](#nccl-debug-file "Permalink to this heading")
...
### NCCL_DEBUG_SUBSYS[](#nccl-debug-subsys "Permalink to this heading")
...
#### Value accepted[](#value-accepted "Permalink to this heading")
...
### NCCL_DEBUG_TIMESTAMP_LEVELS[](#nccl-debug-timestamp-levels "Permalink to this heading")
...
### NCCL_COLLNET_ENABLE[](#nccl-collnet-enable "Permalink to this heading")
...
### NCCL_COLLNET_NODE_THRESHOLD[](#nccl-collnet-node-threshold "Permalink to this heading")
...
### NCCL_CTA_POLICY[](#nccl-cta-policy "Permalink to this heading")
...
For more explanation about NCCL policies, please see [NCCL Communicator CTA Policy Flags](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/flags.html#cta-policy-flags).

### NCCL_NETDEVS_POLICY[](#nccl-netdevs-policy "Permalink to this heading")
...
### NCCL_P2P_DISABLE[](#nccl-p2p-disable "Permalink to this heading")
...
### NCCL_P2P_LEVEL[](#nccl-p2p-level "Permalink to this heading")
...
#### Integer Values (Legacy)[](#integer-values-legacy "Permalink to this heading")
...
### NCCL_P2P_DIRECT_DISABLE[](#nccl-p2p-direct-disable "Permalink to this heading")
...
### NCCL_SHM_DISABLE[](#nccl-shm-disable "Permalink to this heading")
...
### NCCL_BUFFSIZE[](#nccl-buffsize "Permalink to this heading")
...
### NCCL_NTHREADS[](#nccl-nthreads "Permalink to this heading")
...
### NCCL_MAX_NCHANNELS[](#nccl-max-nchannels "Permalink to this heading")
...
### NCCL_MIN_NCHANNELS[](#nccl-min-nchannels "Permalink to this heading")
...
Note that for small message sizes it is possible that NCCL will use fewer channels than NCCL_MIN_CTAS.
...
### NCCL_CHECKS_DISABLE[](#nccl-checks-disable "Permalink to this heading")
...
### NCCL_LAUNCH_MODE[](#nccl-launch-mode "Permalink to this heading")
...
### NCCL_IB_DISABLE[](#nccl-ib-disable "Permalink to this heading")
...
### NCCL_IB_AR_THRESHOLD[](#nccl-ib-ar-threshold "Permalink to this heading")
...
### NCCL_IB_SPLIT_DATA_ON_QPS[](#nccl-ib-split-data-on-qps "Permalink to this heading")
...
### NCCL_IB_CUDA_SUPPORT[](#nccl-ib-cuda-support "Permalink to this heading")
...
### NCCL_IB_PCI_RELAXED_ORDERING[](#nccl-ib-pci-relaxed-ordering "Permalink to this heading")
...
### NCCL_IB_ADAPTIVE_ROUTING[](#nccl-ib-adaptive-routing "Permalink to this heading")
...
### NCCL_IB_ECE_ENABLE[](#nccl-ib-ece-enable "Permalink to this heading")
...
### NCCL_MEM_SYNC_DOMAIN[](#nccl-mem-sync-domain "Permalink to this heading")
...
### NCCL_CUMEM_ENABLE[](#nccl-cumem-enable "Permalink to this heading")
...
### NCCL_CUMEM_HOST_ENABLE[](#nccl-cumem-host-enable "Permalink to this heading")
...
### NCCL_NET_GDR_LEVEL (formerly NCCL_IB_GDR_LEVEL)[](#nccl-net-gdr-level-formerly-nccl-ib-gdr-level "Permalink to this heading")
...
### NCCL_NET_GDR_C2C[](#nccl-net-gdr-c2c "Permalink to this heading")
...
### NCCL_NET_GDR_READ[](#nccl-net-gdr-read "Permalink to this heading")
...
### NCCL_NET_SHARED_BUFFERS[](#nccl-net-shared-buffers "Permalink to this heading")
...
### NCCL_NET_SHARED_COMMS[](#nccl-net-shared-comms "Permalink to this heading")
...
### NCCL_LL_THRESHOLD[](#nccl-ll-threshold "Permalink to this heading")
...
### NCCL_TREE_THRESHOLD[](#nccl-tree-threshold "Permalink to this heading")
...
### NCCL_ALGO[](#nccl-algo "Permalink to this heading")
...
### NCCL_PROTO[](#nccl-proto "Permalink to this heading")
...
### NCCL_NVB_DISABLE[](#nccl-nvb-disable "Permalink to this heading")
...
### NCCL_PXN_DISABLE[](#nccl-pxn-disable "Permalink to this heading")
...
### NCCL_P2P_PXN_LEVEL[](#nccl-p2p-pxn-level "Permalink to this heading")
...
### NCCL_PXN_C2C[](#nccl-pxn-c2c "Permalink to this heading")
...
### NCCL_RUNTIME_CONNECT[](#nccl-runtime-connect "Permalink to this heading")
...
### NCCL_GRAPH_REGISTER[](#nccl-graph-register "Permalink to this heading")
...
### NCCL_LOCAL_REGISTER[](#nccl-local-register "Permalink to this heading")
...
### NCCL_LEGACY_CUDA_REGISTER[](#nccl-legacy-cuda-register "Permalink to this heading")
...
### NCCL_WIN_ENABLE[](#nccl-win-enable "Permalink to this heading")
...
### NCCL_SET_STACK_SIZE[](#nccl-set-stack-size "Permalink to this heading")
...
### NCCL_GRAPH_MIXING_SUPPORT[](#nccl-graph-mixing-support "Permalink to this heading")
...
### NCCL_DMABUF_ENABLE[](#nccl-dmabuf-enable "Permalink to this heading")
...
### NCCL_P2P_NET_CHUNKSIZE[](#nccl-p2p-net-chunksize "Permalink to this heading")
...
### NCCL_P2P_LL_THRESHOLD[](#nccl-p2p-ll-threshold "Permalink to this heading")
...
### NCCL_ALLOC_P2P_NET_LL_BUFFERS[](#nccl-alloc-p2p-net-ll-buffers "Permalink to this heading")
...
### NCCL_COMM_BLOCKING[](#nccl-comm-blocking "Permalink to this heading")
...
### NCCL_CGA_CLUSTER_SIZE[](#nccl-cga-cluster-size "Permalink to this heading")
...
### NCCL_MAX_CTAS[](#nccl-max-ctas "Permalink to this heading")

(since 2.17)

Set the maximum number of CTAs that NCCL should use. Setting this environment variable will override the `maxCTAs` configuration in all communicators (see [ncclConfig_t](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/types.html#ncclconfig)); if not set (undefined), maximum CTAs will be determined by the configuration; if not passing configuration, NCCL will automatically choose the best value.

Increasing the number of CTAs will consume more GPU resources but possibly increase throughput. NCCL normally prioritizes throughput, but on platforms where reaching the peak requires a lot of GPU resources, the maximum number of CTAs may be capped by default. This is expected to provide more balanced performance for GPU-intensive applications, but it is obviously workload-dependent; in particular, for benchmarking purposes, manually boosting the maximum may result in improved throughput numbers.
...
### NCCL_MIN_CTAS[](#nccl-min-ctas "Permalink to this heading")

(since 2.17)

Set the minimum number of CTAs that NCCL should use. Setting this environment variable will override the `minCTAs` configuration in all communicators (see [ncclConfig_t](https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/types.html#ncclconfig)); if not set (undefined), minimum CTAs will be determined by the configuration; if not passing configuration, NCCL will automatically choose the best value.

For each collective operation, NCCL estimates the optimal number of CTAs to use. Should this estimate be overly conservative, increasing the minimum number of CTAs can override it. Note that using more CTAs than necessary can actually decrease the performance, especially for smaller messages, so caution is advised (for very small messages, it is possible that NCCL will use fewer channels than NCCL_MIN_CTAS).

This parameter is commonly used during benchmarking and testing to force a particular number of channels to be used; the minimum and maximum should be set to the same value.
...
### NCCL_NVLS_ENABLE[](#nccl-nvls-enable "Permalink to this heading")
...
### NCCL_IB_MERGE_NICS[](#nccl-ib-merge-nics "Permalink to this heading")
...
2: Automatic detection of MNNVL support. Will _not_ fail if MNNVL is unsupported or if MNNVL resources cannot be allocated.

### NCCL_MNNVL_UUID[](#nccl-mnnvl-uuid "Permalink to this heading")
...
### NCCL_MNNVL_CLIQUE_ID[](#nccl-mnnvl-clique-id "Permalink to this heading")
...
### NCCL_RAS_ENABLE[](#nccl-ras-enable "Permalink to this heading")
...
### NCCL_RAS_ADDR[](#nccl-ras-addr "Permalink to this heading")
...
### NCCL_RAS_TIMEOUT_FACTOR[](#nccl-ras-timeout-factor "Permalink to this heading")
...
### NCCL_LAUNCH_ORDER_IMPLICIT[](#nccl-launch-order-implicit "Permalink to this heading")
...
### NCCL_LAUNCH_RACE_FATAL[](#nccl-launch-race-fatal "Permalink to this heading")
...
### NCCL_IPC_USE_ABSTRACT_SOCKET[](#nccl-ipc-use-abstract-socket "Permalink to this heading")
...
### NCCL_SYM_GIN_KERNELS_ENABLE[](#nccl-sym-gin-kernels-enable "Permalink to this heading")
